...
import UserArtistState from "../../../../../providers/user/encapsulations/lastfm/user.state.artist.class";
...
const mockTypedLastFMHook = mockLastFMHook as userHookAsLastFMTop20ArtistReport;

describe("Top20ReportContainer", () => {
  const testUsername = "niall-byrne";
  let mockHookState: userHookAsLastFMTop20ArtistReport;
  ...
  const checkDataFetching = () => {
    it("should clear the state and request new data", () => {
      ...
      expect(mockHookState.top20artists).toBeCalledTimes(1);
      expect(mockHookState.top20artists).toBeCalledWith(testUsername);
    });
  ...
  const checkFlipCardReportProps = (visible: boolean) => {
    ...
    expect(call.userState).toBeInstanceOf(UserArtistState);
    ...
    expect(call.report).toBeInstanceOf(Top20ArtistsReport);
  ...
  const arrange = () => {
    render(
      <UserInterfaceImageProvider>
        <Top20ArtistsContainer userName={testUsername} user={mockHookState} />
      </UserInterfaceImageProvider>
    );
  };
        ...
        describe("when the images are all loaded", () => {
          ...
          it("should generate an analytics event indicating the report was viewed", () => {
            ...
            expect(mockAnalyticsHook.event).toBeCalledWith(
              Events.LastFM.ReportPresented("TOP20 ARTISTS")
            );
          });
        });
      });
