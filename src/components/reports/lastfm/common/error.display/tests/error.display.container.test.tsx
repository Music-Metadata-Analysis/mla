...
import ConditionalErrorDisplayBase from "../error.conditions/bases/error.condition.base.class";
...
// Error Conditions
jest.mock("../error.conditions/bases/condition.base.class");
...
describe("LastFMErrorDisplayContainer", () => {
  ...
  const checkMockedErrorCondition = (
    mockedComponent: typeof ConditionalErrorDisplayBase,
    calls: number
  ) => {
  ...
  describe("when an error condition is triggered", () => {
    beforeEach(() => {
      mockCondition(ConditionalErrorDisplayBase, <MockError />);

      arrange();
    });

    checkMockedErrorCondition(ConditionalErrorDisplayBase, 1);
  ...
  describe("when no error condition is triggered", () => {
    beforeEach(() => {
      mockCondition(ConditionalErrorDisplayBase, null);

      arrange();
    });

    checkMockedErrorCondition(ConditionalErrorDisplayBase, 5);
