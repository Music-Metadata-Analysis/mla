...
import type { AuthVendorProfilePersistenceClientConstructorType } from "@src/vendors/types/integrations/auth/vendor.backend.types";

export const getGroup = (identifier: unknown) => {
  const hashAsString = JSON.parse(
    (process.env.FLAG_GROUPS_HASH as string) || "{}"
  );
  return new flagVendorBackend.Group(hashAsString).getFromIdentifier(
