import ReportCacheEndpointFactoryBaseV2 from "./methods/generics/v2.report.cache.endpoint.factory.base.class";
import CreateRequestBodyValidatorMiddleware from "./middleware/create.request.body.validator.middleware.class";
import LegacyCreateRequestParamsValidatorMiddleware from "./middleware/legacy.create.request.params.validator.middleware.class";
import AuthenticationMiddleware from "@src/api/services/generics/endpoints/middleware/authentication.middleware.class";
import SetHeaderMiddleware from "@src/api/services/generics/endpoints/middleware/set.header.middleware.class";
...
export const EndpointSunsetDate = new Date(
  "Fri Dec 01 2023 00:00:00 GMT-0500 (Eastern Standard Time)"
);

export default class LegacyReportCacheCreateEndpointFactoryV2 extends ReportCacheEndpointFactoryBaseV2<ReportCacheCreateResponseInterface> {
  ...
  public readonly route = apiRoutes.v2.cacheLegacyCreate;
  ...
  protected override setUpMiddleware(
    ...
    middlewareStack.useBefore(
      new LegacyCreateRequestParamsValidatorMiddleware(this.validators)
    );
    middlewareStack.useBefore(
      new CreateRequestBodyValidatorMiddleware(this.validators)
    );
    middlewareStack.useAfter(
      new SetHeaderMiddleware("Sunset", EndpointSunsetDate.toString())
    );
